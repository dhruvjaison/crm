...
export function CSVImportDialog({ open, onOpenChange, onImportComplete }: CSVImportDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/contacts/import', {
        ...
        body: JSON.stringify({ contacts, updateExisting }),
      })
  ...
  const downloadTemplate = () => {
    const template = 'firstName,lastName,email,phone,company,jobTitle,status,tags,notes\nJohn,Doe,john@example.com,+1234567890,Acme Inc,CEO,LEAD,"tag1,tag2",Sample notes'
              ...
              {file && (
                <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      If a contact with the same email/phone exists, update it
                    </p>
                  </div>
                  <Switch
                    id="update-existing"
